    ...
    */
    ...
    if (pid == -1) {
      ...
      while ((read = getline(&line, &len, file)) != -1) {
          ...
          printf("%s\n", tmpPath);
          char *tmpTmpPath = strdup(tmpPath);
          char *hash = getResultForFile(tmpTmpPath, "md5sum");
          printf("%s\n", tmpPath);
          char *tmpSecondPath = strdup(tmpPath);
          char *fileType = getResultForFile(tmpSecondPath, "file");
