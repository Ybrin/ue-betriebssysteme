...
static void insert_after(struct listelem *after, const char *const value)
{
    ...
    struct listelem *inserted = malloc(sizeof(struct listelem));
    inserted->val = strdup(value);
...
int main(int argc, char **argv)
{
    ...
    if (opt_s != -1) {
        for (struct listelem *currentListElem = head; currentListElem != NULL; currentListElem = currentListElem->next) {            /* TODO: change it */
            if (STREQ(currentListElem->val, optstr)) {  /* TODO: change it */
                printf("yes,");
            } else {
                printf("no,");
            }
    ...
    if (opt_a != -1) {
        ...
        current = head;
        int count = 0;
        for (; current->next != NULL; current = current->next) {            /* TODO: change it */
          // printf("%s\n%d\n%d\n", "INSIDE LOOP", count, numA);
          if (count == numA) {
            // printf("%s\n", "YAU");
            break;
          }
