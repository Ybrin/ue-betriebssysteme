...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (sockfd == -1) {
      bail_out(EXIT_FAILURE, "TCP/IP socket creation impossible...");
    }
    int optval = 1;
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) == -1) {
      bail_out(EXIT_FAILURE,
        "The method setsockopt failed. We don't have more information about this incident. SO_REUSEADDR option can't be set...");
    }
    ...
    server_address.sin_port = htons(options.portno);

    if (bind(sockfd, (const struct sockaddr *) &server_address, sizeof(server_address)) == -1) {
      bail_out(EXIT_FAILURE, "Can't bind socket to port: %d", options.portno);
    }

    if (listen(sockfd, BACKLOG) == -1) {
      bail_out(EXIT_FAILURE, "Can't listen on socket...");
    }

    connfd = accept(sockfd, (struct sockaddr *) &client_address, &client_address_size);
    if (connfd == -1) {
      bail_out(EXIT_FAILURE, "Can't accept the client connection...");
    }
    ...
    for (round = 1; round <= MAX_TRIES && !quit; ++round) {
        ...
        // TODO #error "insert your code here"
        if (send(connfd, &buffer[0], sizeof(buffer[0]), 0) < 0) {
          bail_out(EXIT_FAILURE, "Can't send message to client...");
        }
